...
export function setupGUI(parentContext) {
  ...
  // ─── Dance / Mimic Mode Selection ─────────────────────────────────────────
  const sendDanceCmd = (cmd) => {
    if (!parentContext.ws || parentContext.ws.readyState !== 1) {
      alert('Bridge not connected! Start g1_ws_bridge first.');
      return;
    }
    parentContext.ws.send(JSON.stringify({ cmd }));
    console.log('[Dance] Sent cmd:', cmd);
  };

  const danceFolder = parentContext.gui.addFolder('🕺 Dance / Mimic');

  // Status label
  const danceStatus = { status: 'Idle — enter FixStand, wait 6s → Velocity' };
  const danceStatusCtrl = danceFolder.add(danceStatus, 'status').name('Status').listen();
  danceStatusCtrl.domElement.querySelector('input').disabled = true;

  danceFolder.add({ dance102: () => {
    sendDanceCmd('dance102');
    danceStatus.status = ' Dance 102 triggered';
  }}, 'dance102').name('Dance 102');

  danceFolder.add({ gangnam: () => {
    sendDanceCmd('gangnam');
    danceStatus.status = ' Gangnam Style triggered';
  }}, 'gangnam').name('Gangnam Style');

  //Add more action

  danceFolder.open();

  // Reset status on scene reload
  parentContext.updateGUICallbacks.push(() => {
    danceStatus.status = parentContext.wsBridge ? 'Idle — enter FixStand, wait 6s → Velocity' : '🔴 Bridge offline';
  });
  // ──────────────────────────────────────────────────────────────────────────
...
export async function downloadExampleScenesFolder(mujoco) {
  let allFiles = [
    ...
    "g1/meshes/left_hand_index_0_link.STL",
    "g1/meshes/left_hand_index_1_link.STL",
    "g1/meshes/left_hand_middle_0_link.STL",
    "g1/meshes/left_hand_middle_1_link.STL",
    "g1/meshes/left_hand_palm_link.STL",
    "g1/meshes/left_hand_thumb_0_link.STL",
    "g1/meshes/left_hand_thumb_1_link.STL",
    "g1/meshes/left_hand_thumb_2_link.STL",
    ...
    "g1/meshes/left_wrist_pitch_link.STL",
    "g1/meshes/left_wrist_roll_link.STL",
    "g1/meshes/left_wrist_roll_rubber_hand.STL",
    "g1/meshes/left_wrist_yaw_link.STL",
    ...
    "g1/meshes/right_hand_index_0_link.STL",
    "g1/meshes/right_hand_index_1_link.STL",
    "g1/meshes/right_hand_middle_0_link.STL",
    "g1/meshes/right_hand_middle_1_link.STL",
    "g1/meshes/right_hand_palm_link.STL",
    "g1/meshes/right_hand_thumb_0_link.STL",
    "g1/meshes/right_hand_thumb_1_link.STL",
    "g1/meshes/right_hand_thumb_2_link.STL",
    ...
    "g1/meshes/right_rubber_hand.STL",
    "g1/meshes/right_shoulder_pitch_link.STL",
    "g1/meshes/right_shoulder_roll_link.STL",
    "g1/meshes/right_shoulder_yaw_link.STL",
    "g1/meshes/right_wrist_pitch_link.STL",
    "g1/meshes/right_wrist_roll_link.STL",
    "g1/meshes/right_wrist_roll_rubber_hand.STL",
    "g1/meshes/right_wrist_yaw_link.STL",
    "g1/meshes/torso_constraint_L_link.STL",
    "g1/meshes/torso_constraint_L_rod_link.STL",
    "g1/meshes/torso_constraint_R_link.STL",
    "g1/meshes/torso_constraint_R_rod_link.STL",
    "g1/meshes/torso_link.STL",
    "g1/meshes/waist_constraint_L.STL",
    "g1/meshes/waist_constraint_R.STL",
    "g1/meshes/waist_roll_link.STL",
    "g1/meshes/waist_support_link.STL",
  ...
  // G1 scene.xml references height_field.png as "../height_field.png" (relative to g1/ dir)
  // so we also write them at /working/ root level.
  let hfPairs = [
    ["./assets/scenes/g1/height_field.png",   "/working/height_field.png"],
    ["./assets/scenes/g1/unitree_hfield.png", "/working/unitree_hfield.png"],
  ];
  for (let [fetchPath, writePath] of hfPairs) {
    let resp = await fetch(fetchPath);
    mujoco.FS.writeFile(writePath, new Uint8Array(await resp.arrayBuffer()));
  }
}
